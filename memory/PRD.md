...
- FEATURE (lanjutan) Rapikan seluruh menu ekspor sesuai standar administrasi & laporan: /export/{jenis} ditulis ulang total. Kolom dikurasi & berlabel Indonesia (bukan key mentah DB); status/prioritas/kelompok/tanggal diformat. Tiap laporan punya KOP (judul, puskesmas, periode+kelurahan, "Dicetak: tgl oleh user"). Excel: judul merge, header teal tebal + border, lebar kolom auto, freeze header, total. PDF: kop tengah, header tabel berulang tiap halaman, border, wrap teks, "Total data", blok tanda tangan (Mengetahui, Kepala Puskesmas). CSV: BOM UTF-8 + kop + tabel. Nama file berstempel tanggal. Tambah jenis "Rekap Tindak Lanjut Pustu". Verified 4 jenis x 3 format = 12 semua HTTP 200 & file valid (cek visual PDF+Excel).
